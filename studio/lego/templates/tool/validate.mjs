/*
 * __NAME__ — the VALIDATOR for capability "__CAPABILITY__".  THE GATE.
 *
...
for (const decl of DECLARED_OUTPUTS) {
  ...
  if (!fs.existsSync(abs)) {
    ...
    if (decl.endsWith('.json')) {
      fs.writeFileSync(abs, JSON.stringify({ capability: '__CAPABILITY__', placeholder: true }, null, 2) + '\n');
    } else {
...
const verdict = { pass, score, notes, capability: '__CAPABILITY__' };
